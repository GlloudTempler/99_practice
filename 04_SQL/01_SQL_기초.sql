-- sql-developer 툴 : DB접속툴(===DB 클라이언트 툴)
-- oracle 설치     : DB서버
-- DB 서버 <- 사용자가 접속해서 사용함(계정/암호)
-- ststem계정 : 관리자(계정생성, 권한관리 또는 DB 환경관리 용으로 사용)
-- scott 계정 : 개발자(사용자) 계정 - 백엔드 개발을 진행
-- sql-developer 툴 단축키:
-- 1) 주석              : ctrl + /
...
-- 예제 2) 사원테이블의 컬럼중에서 ENO, ENAME 컬럼만 정보를 보는 SQL문을 작성
...
-- 예제 4) 힌트) 컬럼에 *(/,+,-:산술연산자) 연산해서 화면에 출력할 수 있다.
-- 사원테이블 : EMPLOYEE
-- 월급컬럼 : SALARY
-- 사원명 : ENAME
...
-- 사원테이블에 부서번호(DNO) 중복을 제거해서 화면에 출력
