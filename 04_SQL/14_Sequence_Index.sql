...
-- 1. 시퀀스 : 호출, 실행 할때마다 자동적으로 숫자를 증가시키는 것(객체)
-- 목적 : 기본키 용도(중복없음, null없음)

-- 활용 예) 자유게시판 : No(1~n), 고객: No(1~n)
-- 실무 : 채번 : 자동적으로 1씩 증가시키는 값(1~n : 시퀀스 대상)
...
-- 현재 증가된 시퀀의 값을 확인 - 최초 1번은 시퀀스.NEXTVAL을 실행시켜야함
-- 사용법) 시퀀스명.CURRVAL
...
-- 임시 테이블(구조) : DEPARTMENT -> 복사 DEPT_TEMP
...
-- 목적) 조회속도 향상을 위해 조건에 해당하는 컬럼에 인덱스를 생성함(지정)
...
-- 사용법) CREATE INDEX 인덱스명 ON 대상테이블(대상칼럼);
CREATE INDEX IX_EMPLOYEE_ENAME ON EMPLOYEE(ENAME);

-- 속도 느림 대상 SQL문
SELECT * FROM EMPLOYEE
WHERE ENAME LIKE 'SCOTT%'; -- 이부분이 INDEX를 만들 곳

-- 데이터사전 : USER_IND_COLUMNS
...
-- ! 인덱스를 만들어야 하는 기준
-- 1) 테이블의 행의 개수가 많을 경우(데이터 건수가 많을 경우)
--    : 약 10만건 이상
