...
BEGIN
    -- 2) 실행부
    -- 사용법) SELECT 컬럼명 INTO 변수명 FROM 테이블명 : 컬럼값 -> 변수로 저장
...
-- 선언부(정의부)
DECLARE
        -- CURSOR === 2차원 배열 : 여러 데이터를 가지고 있는 SELECT 조회문
...
BEGIN
    ...
    -- 증감식 없음, 데이터의 끝에 도달하면 반복문 종료
    -- 사용법)
    -- FOR 변수명(객체) IN 커서 변수명(배열) LOOP
    --  반복문(변수명.컬럼명);
    -- END LOOP;
...
-- (매개변수 in 자료형)
-- (내보내기변수 out 자료형) -- js return값이랑 비슷
...
-- 예제 7) 펑션 만들기
-- 부서번호를 매개변수ㅗㄹ 받아서 위치를 출력하는 함수 정의
-- 함수의 특징 : 무조건 return 값이 있음(결과 내보내기가 있어야함)
-- 사용법)
